...
(function( win, undefined ){
	 ...
	 */
            createDialogBox : function(){
				/*var tmpObj = document.getElementById('easyDialogBox');
 ...
 */
        ...
        var extend = {
            open : function(){
                ...
                /*
                jQuery('.easyDialog_wrapper').css('height', options.container.height+30+'px');
                jQuery('.easyDialog_text').css('height', options.container.height+'px');
                jQuery('.easyDialog_wrapper').css('width', options.container.width+'px');
                */

                jQuery('.easyDialog_wrapper').height(options.container.height+80);
                jQuery('.easyDialog_text').height(options.container.height);
                jQuery('.easyDialog_wrapper').width(options.container.width+2);
...
//另一个弹出框封装
function popEasyDialog(option){
    var containerOpt = {
        width : option.width
        ,height : option.height
        ,header : option.title
        ,content : option.content
        ,noText : option.btnNoText
    };
    if(option.cancleCallBack!==undefined){
		containerOpt.noFn = option.cancleCallBack;
	}
    if(option.confirmCallBack!==undefined){
		containerOpt.yesFn = option.confirmCallBack;
	}
